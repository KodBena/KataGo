  ...
  for(size_t i = argStart; i<args.size(); i++) {
    ...
    if(a == "-trace" && hasNext) tracePath = args[++i];
      ...
      throw StringError(
        ...
        "                     regime a replacement rule exists for. A saturated ghost does not\n"
        "                     merely blur the two rules, it REPLACES them: a newcomer's own\n"
        "                     sighting rewrites its slot just before the comparison so it reads 1,\n"
        "                     while an overwritten incumbent reads 0, which makes keepmoreseen\n"
        "                     admit unconditionally -- it becomes `always` -- and keeplessseen\n"
        "                     refuse unconditionally. Such a sweep measures neither rule. This was\n"
        "                     measured on this branch, not feared.\n"
